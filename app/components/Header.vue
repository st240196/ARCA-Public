<template>
    <header class="sticky flex top-0 items-center w-full h-16 p-2 bg-white shadow-md z-40">
        <NuxtLink class="w-20 h-20 ml-2 mr-6 transition-all duration-300 hover:scale-120" to="/">
        ...
        <nav class="hidden sm:flex bg-gradient-to-r from-orange-500 via-lime-500 to-sky-500 bg-clip-text text-transparent font-bold flex-row gap-6">
            <NuxtLink class="p-2 text-xl hover:text-white/40" to="/">ホーム</NuxtLink>
            <NuxtLink class="p-2 text-xl hover:text-white/40" to="/stats">統計</NuxtLink>
            <NuxtLink class="p-2 text-xl hover:text-white/40" to="/faq">FAQ</NuxtLink>
            <NuxtLink class="p-2 text-xl hover:text-white/40" to="/credit">クレジット</NuxtLink>
        </nav>
        ...
        <div class="fixed inset-0 sm:hidden transition-all duration-300 z-40" :class="open ? 'pointer-events-auto' : 'pointer-events-none'">
            ...
            <div class="absolute p-4 top-0 w-64 h-full bg-white rounded-tl-2xl rounded-bl-2xl shadow-lg transition-all duration-300" :class="open ? 'right-0' : 'right-[-32rem]'" @click.stop>
                <nav class="flex flex-col bg-gradient-to-b from-orange-500 via-lime-500 to-sky-500 bg-clip-text text-transparent mt-10 gap-3">
                    <NuxtLink class="p-2 text-xl font-bold hover:text-white/40" @click="close" to="/">ホーム</NuxtLink>
                    <NuxtLink class="p-2 text-xl font-bold hover:text-white/40" @click="close" to="/stats">統計</NuxtLink>
                    <NuxtLink class="p-2 text-xl font-bold hover:text-white/40" @click="close" to="/faq">FAQ</NuxtLink>
                    <NuxtLink class="p-2 text-xl font-bold hover:text-white/40" @click="close" to="/credit">クレジット</NuxtLink>
